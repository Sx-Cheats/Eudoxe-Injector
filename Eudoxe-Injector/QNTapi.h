...
public:
    QNTapi()
    {
        ...
        CurrentProcess = this->GetProcessInfo(GetCurrentProcessId());

    }

    [[nodiscard]] static PSYSTEM_PROCESS_INFORMATION WINAPI GetProcessInfo(DWORD pid);
    [[nodiscard]] static EnumObj WINAPI GetProcesses();
    [[nodiscard]] static PSYSTEM_PROCESS_INFORMATION WINAPI GetProcessInfo(std::string appname);
    [[nodiscard]] static EnumObj WINAPI GetAllProcessInfo(std::string ProcessName);
    static DEFAULT_SIZE WINAPI GetModuleAddressA(std::string ModuleName);
    ...
    static bool WINAPI ModuleExist(DEFAULT_SIZE base);
    ...
    [[nodiscard]] static MODULE_ENTRY WINAPI GetModulesEx(const char* AppName);
    [[nodiscard]] static MODULE_ENTRY WINAPI GetModulesEx(DWORD pid);
    static DWORD WINAPI SetPageAccess(void* base, DWORD ID);
    static HWND  GetWindowByPid(DWORD pid);
    static void SetWindowFocus(DWORD pid);
    static PEB peb;

private:
    [[nodiscard]] static MODULE_ENTRY WINAPI __get_modules_ex__(DWORD pid);
    template<typename cb>
    static void WINAPI __enum_process__(cb callback);
    typedef struct __window_callback_arg_
    {
        DWORD  pid = 0;
        HWND   hwnd = 0;
    };
